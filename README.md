...
> 本项目配置定位清晰，主要用于个人算法竞赛，（Windowss 版本）上传至 Github 仓库留作备份。写的很杂乱，仍有许多地方不完善。



**有关为什么我会选用 Vim，而不是其它主流编辑器如 VSCode？**

1. 轻量快速：打开任何文件秒启动，占用内存仅几十 MB，再差的配置也能启动。对比其它编辑器，启动慢，占用内存动不动就几百 MB 甚至几 G。
2. 存储空间低：精简/压缩后的 Vim 占用空间只有几 MB，可以快速在设备之间拷贝。
3. 开箱即用：下载完直接解压就能用，且保留配置文件，无需二次配置。
4. 跨平台：支持 Linux、MacOS、Windows，也可以在手机/平板运行（有一个终端就行）。
5. 输入效率高：全键盘操作熟练后，可以专注于实现算法本身，思路基本不会被输入打断。
...
├── template/            # 50+ 个 ACM 算法模板（m-*.cpp）与字典补全
...
`mexgcd` 扩展欧几里得 | `mmanacher` 马拉车回文串 | `msplay` Splay 树 | `mdlx` Dancing Links 舞蹈链 | `mgeo` 计算几何封装 
`mexcrt` 扩展中国剩余定理 | `mtrie` 字典树 | `mmat_tree` 矩阵树 | `mst` 稀疏表 | `mfft` / `mntt` FFT/NTT
`mlcm` 最小公倍数 | `mxor_trie` 异或字典树 | `mlca` 最近公共祖先 | `mrng` 随机数生成器 | `mfrac` 分数封装 
`misprime` 素数判断 | `macam` AC自动机 | `m-lowbit` / `mlb` 树状数组 | `mpollard_rho` Pollard-Rho | `mlog` 对数
`mget_phi` 线性欧拉函数筛 | `mget_primes` 线性素数筛 | `mget_inv` 逆元 | `mget_fact` 阶乘 | `mfact` 阶乘/组合数 
`mfib` 斐波那契 | `m-exkmp` 扩展KMP | `mgauss` 高斯消元 | `mgauss_xor` 异或线性基 | `mpc` 排列/组合枚举 
`mread` 快读 | `mmat` 矩阵 | `msegt` 线段树 | | 
