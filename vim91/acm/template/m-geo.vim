...
int parallel(const Line<T> &l1,const Line<T> &l2){
	if(cross(l1.b-l1.a,l2.b-l2.a) == 0){
		if(distancePL(l2.b,l1) == 0) return 1;//sameLine
...
template<typename T>
bool pointInPolygon(const Point<T> &a, const std::vector<Point<T>> &p) {//0_idx
    int n = p.size();
    for (int i = 0; i < n; i++) {
...
template<typename T>
bool segmentInPolygon(const Line<T> &l, const std::vector<Point<T>> &p) {//0_idx
	int n = p.size();
...
template<typename T>
std::vector<Point<T>> hp(std::vector<Line<T>> lines) {//0_idx
